...
import { MdDeleteOutline, MdEdit, MdSend } from "react-icons/md";
...
const UsersLog: React.FC<UsersLogProps> = ({ isLoading }) => {
  ...
  const handleResendOTP = (item: UsersLogItem) => {
    ...
    setTimeout(() => {
      console.log(
        { email: item?.email, resend_type: "add_staff" },
        "payload_for_Unyime"
      );
      dispatch(resendOTP({ email: item?.email, resend_type: "add_staff" }))
        .then((status) => {
          setResendTokenLoading((prevLoadingMap) => ({
            ...prevLoadingMap,
            [item.id]: false,
  ...
  const handleDeleteStaff = async () => {
    try {
      ...
      switch (response?.payload) {
        ...
        case 403:
          setLoading(false);
          setFormErrors("Sorry only super admins are allowed to do this.");
          break;
        default:
          setLoading(false);
  ...
  const handleSecondModalContinue = async () => {
    ...
    if (submitErrors) {
      try {
        ...
        console.log(response?.payload, "response?.payloadresponse?.payload");
        switch (response?.payload) {
          case 200:
            console.log("Profile successfully updated:", response);
            setFormData({
    ...
    try {
      ...
      switch (response?.payload) {
        case 200:
          ...
          break;
        case 400:
          setFormErrors("Sorry this organization can only have 2 managers");
          break;
        case 422:
          setFormErrors("Please Enter Details Correctly to Proceed.");
          break;
        ...
        <div>
          <p className="name-users-style-bold-email">
            {clickedUser?.permission_type.toLowerCase() === "executive"
              ? "SE Executive"
              : null}
            {clickedUser?.permission_type.toLowerCase() === "manager"
              ? "SE Manager"
              : null}
            {clickedUser?.permission_type.toLowerCase() === "support"
              ? "SE Support"
              : null}
          </p>
        </div>
    ...
    <div>
      <div>
        ...
        >
          ...
          <Modal
            ifClose={true}
            isOpen={isModalOpen} // or any other state variable you want to use
            onClose={closeModal} // or any other close function corresponding to the above state variable
            formContent={content} // or any other form content corresponding to the above state variable
          />
        </div>
      </div>
    </div>
